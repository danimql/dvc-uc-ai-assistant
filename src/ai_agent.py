...
def main():
    ...
    while True:
        ...
        if again not in ("y", "yes"):
            print("Goodbye! 👋")
            break
# --- Minimal adapter so Flask can call our logic ---
from pathlib import Path

def run_my_logic(user_input: str) -> str:
    """
    Stateless entrypoint used by app.py.
    Takes a free-text prompt and returns the formatted multi-campus response.
    """
    # Make sure env is loaded (works when run via Flask or CLI)
    load_dotenv(Path(__file__).resolve().parents[1] / ".env")

    api_key = os.getenv("OPENAI_API_KEY")
    client = OpenAI(api_key=api_key) if api_key else OpenAI()

    # Load any campus JSONs present (your filenames like ucb_25-26.json, ucd_24-25.json, ucsd_24-25.json)
    data = load_all_data([
        os.path.join("agreements_25-26", "*.json"),
        os.path.join("data", "uc*.json"),
    ])
    if not data:
        return "No campus files loaded. Check agreements_25-26/ or data/."

    # Parse the user message to get campuses + filters
    parsed = llm_parse_user_message(client, user_input)

    # Decide which campuses to include
    campus_keys = parsed.get("parameters", {}).get("campuses") or detect_campuses_from_query(user_input)
    if not campus_keys:
        # default to all we have
        campus_keys = sorted(list(data.keys()))
    # keep only those we actually loaded
    campus_keys = [ck for ck in campus_keys if ck in data]
    if not campus_keys:
        return "Could not find data for the requested campus(es)."

    # Build rows and apply filters (non-interactive)
    seed_prefs = parse_preferences_seed(user_input)
    ...
    required_only = bool(parsed["filters"].get("required_only", False))
    categories_only = parsed["filters"].get("categories") or seed_prefs.get("seed_categories") or []

    campus_to_rows = {}
    for ck in campus_keys:
        all_rows = collect_course_rows(data[ck])
        kept = filter_rows(
            all_rows,
        ...
        campus_to_rows[ck] = kept

    # You can switch to plain=True to avoid an extra LLM formatting call
    return llm_format_response_multi(
        client,
        campus_keys,
        campus_to_rows,
        parsed,
